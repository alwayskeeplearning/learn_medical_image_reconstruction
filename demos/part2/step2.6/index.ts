...
const fragmentShader = `
    ...
    void main() {
        // 使用 texture2D 函数从纹理中采样
        vec4 textureValue = texture2D(u_dicomTexture, vUv);

        // 因为我们的纹理是 RedFormat，CT 值存储在 r 分量中
        float ctValue = textureValue.r;

        // 这是一个非常临时的、简陋的窗宽窗位实现
        float normalizedValue = (ctValue + 1024.0) / 4096.0;

        // gl_FragColor 是内置的特殊变量，表示当前片元的最终颜色
...
const material = new THREE.ShaderMaterial({
  ...
  uniforms: {
    // 在这里定义 uniform 变量
    u_dicomTexture: { value: null }, // 初始值为空
  },
  side: THREE.DoubleSide, // 同样设置为双面可见
});
...
async function loadDicom() {
  ...
  const slope = dataSet.floatString('x00280053', 0) || 1;
  const intercept = dataSet.floatString('x00280052', 0) || 0;
