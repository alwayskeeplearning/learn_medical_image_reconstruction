...
interface ViewConfig {
  ...
  canvas: HTMLCanvasElement; // 专属的 canvas
  renderer: THREE.WebGLRenderer; // 专属的渲染器
  scene: THREE.Scene;
...
async function main() {
  ...
  const axialConfig: Partial<ViewConfig> = {
    ...
    canvas: axialCanvas,
    renderer: new THREE.WebGLRenderer({ canvas: axialCanvas }),
  ...
  const sagittalConfig: Partial<ViewConfig> = {
    ...
    renderer: new THREE.WebGLRenderer({ canvas: sagittalCanvas }),
    scene: new THREE.Scene(),
  ...
  const coronalConfig: Partial<ViewConfig> = {
    ...
    renderer: new THREE.WebGLRenderer({ canvas: coronalCanvas }),
    scene: new THREE.Scene(),
...
function updateView(view: ViewConfig) {
  const { element, renderer, camera, mesh } = view;
...
function animate() {
  ...
  // --- 核心渲染逻辑 (大幅简化) ---
  // 不再需要 setScissorTest, setViewport, setScissor
  views.forEach(view => {
