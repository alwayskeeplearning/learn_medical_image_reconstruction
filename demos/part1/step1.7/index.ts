...
// --- 状态管理：我们需要在事件监听函数外部访问这些变量 ---
let currentDicomInfo: DicomInfo | null = null;
...
fileInput.addEventListener('change', event => {
  ...
  reader.onload = e => {
    try {
      ...
      // 将解析出的信息存入全局变量，以便交互时使用
      currentDicomInfo = dicomInfo;

      // 2. 将解析出的信息显示在页面上
      displayInfo(currentDicomInfo);

      // 3. 渲染图像
      renderImage(currentDicomInfo);
    } catch (error) {
      handleError(error);
    }
  };
...
canvas.addEventListener('mousemove', event => {
  if (isDragging && currentDicomInfo) {
    const deltaX = event.clientX - lastMousePosition.x;
    const deltaY = event.clientY - lastMousePosition.y;

    // 根据鼠标移动的距离调整窗宽窗位
    // 这里的乘数可以调整交互的灵敏度
    currentDicomInfo.windowWidth += deltaX * 2;
    currentDicomInfo.windowCenter += deltaY * 2;

    // 确保窗宽不为负数
    if (currentDicomInfo.windowWidth < 1) {
      currentDicomInfo.windowWidth = 1;
    }
    ...
    requestAnimationFrame(() => {
      renderImage(currentDicomInfo as DicomInfo);
      displayInfo(currentDicomInfo as DicomInfo);
    });
  }
});
